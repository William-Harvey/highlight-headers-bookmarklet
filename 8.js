javascript:(function(){
    ...
    document.querySelectorAll('h1,h2,h3,h4,h5,h6').forEach(h => {
        ...
        // FORCING a skip error between <h2> and <h4> just for testing:
        if (tagName === 'h2' && lastHeadingLevel === 1) {
            // Simulating a skip: Forcing a suggestion after the first <h2>
            suggestion.textContent = `Suggestion: Should be <h3> to avoid skipping levels.`;
            h.style.outline = '2px solid orange';
            h.appendChild(suggestion);  // Append suggestion
            console.log(`Forced skip warning for ${tagName}`);
        } else if (h1Found) {
